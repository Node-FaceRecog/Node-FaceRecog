...
var questions = [{question: 'a', ans: true}, 
  {question: 'b', ans: false}, 
  {question: 'c', ans: true}, 
  {question: 'd', ans: true},
  {question: 'e', ans: true}, {question: 'f', ans: true}, {question: 'g', ans: true}, {question: 'h', ans: true}];
...
function createGame(){
  ...
  header.innerHTML = "Let's play some game while we authenticate you";
  gameContainer.appendChild(header);
  

  var questionDiv = document.createElement('div');
  questionDiv.id = "questionDiv";
  questionDiv.innerHTML = "Question: " + questions[0].question;
  gameContainer.appendChild(questionDiv);
  questioncounter++;
  var radioInputYes = document.createElement('input');
  radioInputYes.setAttribute('type', 'radio');
  radioInputYes.setAttribute('name', "answerchoice");
  radioInputYes.setAttribute('value', "1");
  radioInputYes.addEventListener("change", answserSelected);
  var radioInputNo = document.createElement('input');
  radioInputNo.setAttribute('type', 'radio');
  radioInputNo.setAttribute('name', "answerchoice");
  radioInputNo.setAttribute('value', "0");
  radioInputNo.addEventListener("change", answserSelected);
  gameContainer.appendChild(radioInputYes);
  gameContainer.appendChild(radioInputNo);
  var resultDiv = document.createElement('div');
  resultDiv.id = "resultDiv";
  resultDiv.innerHTML = "0";
  gameContainer.appendChild(resultDiv);
  var modalMsg = document.getElementById('modalMsg');
  //var video = modalMsg.querySelector('video');
  //modalMsg.removeChild(video);

  modalMsg.appendChild(gameContainer);

  gameRunning = true;
}

function answserSelected(e){
  if (questioncounter == questions.length){
    questioncounter = 0;
  }
  e.srcElement.checked = false;
  var currentQuestion = questions[questioncounter];
  if(currentQuestion.ans == e.srcElement.defaultValue){
    var resultDiv = document.getElementById('resultDiv');
    resultDiv.innerHTML = parseInt(resultDiv.innerHTML) != NaN? parseInt(resultDiv.innerHTML)+1: 0;
  }
  
  var questionDiv = document.getElementById('questionDiv');
  questionDiv.innerHTML = "Question: "+ questions[questioncounter].question;
  questioncounter++;
}
...
var gameRunning = false;
  ...
  io.connect = function (host, options) {
    createModal();
    if(options.showGameWhileLoading){
        createGame();
    }
